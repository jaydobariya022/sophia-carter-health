...
var CLICKBANK_BASE_PARAMS = ['vendor', 'affiliate', 'cbpage', 'affop', 'lid'];
...
// ── Mobile Sticky Floating CTA Button ──────────────────────────────────────────
(function initStickyCTA() {
  var ctaBtn = document.getElementById('cta-main-btn');
  var ctaContainer = document.querySelector('.section-cta');
  if (!ctaBtn || !ctaContainer) return;

  function handleScroll() {
    // Only apply sticky behavior on mobile view (screen width <= 767px)
    if (window.innerWidth > 767) {
      ctaBtn.classList.remove('is-sticky');
      return;
    }

    var scrollY = window.scrollY || window.pageYOffset;
    var containerRect = ctaContainer.getBoundingClientRect();
    var windowHeight = window.innerHeight;

    // Sticky float after scrolling down 250px,
    // and un-stick (dock back into inline position) when original CTA container is in viewport bottom
    var isPastStart = scrollY > 250;
    var isContainerVisible = containerRect.top <= (windowHeight - 20);

    if (isPastStart && !isContainerVisible) {
      ctaBtn.classList.add('is-sticky');
    } else {
      ctaBtn.classList.remove('is-sticky');
    }
  }
